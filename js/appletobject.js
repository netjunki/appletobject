				...
				for (i=0; i < navigator.mimeTypes.length; i++) {
					...
					if( (mimeType != null)
						...
						"application/x-java-applet;jpi-version=") != -1) ) {
							...
							var tmpJavaVersion = new AppletObjects.JavaVersion(mimeType.substring(versionIndex+8));
							if(tmpJavaVersion.isGreater(javaVersion)){
								javaVersion = new AppletObjects.JavaVersion(mimeType.substring(versionIndex+8));
							}
							pluginDetected = true;
					}
				}
			...
			if(this.readRegistry) {
				...
				try {
					...
					if (shell != null) {
						...
						try {
							javaVersion = new AppletObjects.JavaVersion(shell.regRead("HKEY_LOCAL_MACHINE\\Software\\JavaSoft\\Java Runtime Environment\\CurrentVersion"));
...
var AppletObject = 
function AppletObject ( /* class, archives[], width, height, minimumVersion, mayscript, codebase, params[], type */ ) 
{
